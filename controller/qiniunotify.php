...
class qiniunotify extends Base
{
	public function post(){
		...
		if(array_key_exists('code',$data) && $data['code'] == 0){
			sae_debug("code is 0");
			$mysql = new MySqlUtil();
			$sql = 'SELECT id,key_preview,key_thumb FROM UploadFile WHERE persistent_id = :persistent_id';
			$params = array(':persistent_id'=>$persistentId);
			$dataset = $mysql->query($sql,$params);
			foreach ($dataset as $raw){
				sae_debug('get id is: ' . $raw['id']);
				if(!$raw['id']) continue;
				$sql = 'UPDATE UploadFile SET key_preview=:key_preview,key_thumb=:key_thumb,is_processing=:is_processing WHERE id=:id';
				$params = array(
					':id'=>$raw['id'],
