...
class resources extends Base
{
	public function get()
	{
		...
		{
			$sql = 'SELECT * FROM UploadFile WHERE parent_id = :pid';
			$params[':pid'] = $pid;
		}
		else
		{
			$sql = 'SELECT * FROM UploadFile WHERE parent_id is null';
		}
		$dataset = $mysql->query($sql,$params);
		foreach ($dataset as $raw){
			$download_url = $raw['type'] == 0 ? null : 
			    ("http://" . $raw['bucket'] . ".qiniudn.com/" . $raw['key_orignal'] . "?download/" . $raw['file_name']); 
			$thumb_url = ($raw['type'] == 0 || $raw['key_thumb'] == null || $raw['key_thumb'] == '') ? null : ("http://" . $raw['bucket'] . ".qiniudn.com/" . $raw['key_thumb']);
			$preview_url = ($raw['type'] == 0 || $raw['key_preview'] == null || $raw['key_preview'] == '') ? null : ("http://" . $raw['bucket'] . ".qiniudn.com/" . $raw['key_preview']); 
	...
	public function post()
	{
		...
		{
			$sql = 'INSERT INTO UploadFile (file_name, owner_account_id, type, create_time, mod_time) VALUES (:file_name, 490, 0, NOW(), NOW())';
			$params[':file_name'] = $file_name;
		}
		else
		{
			$sql = 'INSERT INTO UploadFile (file_name, owner_account_id, type, create_time, mod_time, parent_id) VALUES (:file_name, 490, 0, NOW(), NOW(), :parent_id)';
			$params[':file_name'] = $file_name;
	...
	public function patch()
	{
		...
		$mysql = new MySqlUtil();
		$sql = 'UPDATE UploadFile SET mod_time = NOW()';
		$params = array();
	...
	public function delete()
	{
		...
		$mysql = new MySqlUtil();
		$sql = 'SELECT key_orignal FROM UploadFile WHERE id = :id';
		$params = array(':id'=>$id);
		$dataset = $mysql->query($sql,$params);
		global $BUCKET;
		foreach ($dataset as $raw){
	...
	private function recursioncheck($sourceid, $targetid)
	{
		...
		{
			$result = array(
				'Message' => 'the data in db table UploadFile is error.'
			);
			$this->_Response->sendResponse(405,json_encode($result),'application/json');
		}
